          ...
          async.eachSeries(needed, function(tag, callback) {
            repo.checkout(tag, function() {
              ...
              _.each(library.autoupdate.fileMap, function(mapGroup) {
                 ...
                 _.each(mapGroup.files, function (cRule) {
                     ...
                     if (newFiles.length === 0) {
                      ...
                      fs.mkdirsSync(path.normalize(path.join(__dirname, '../../', 'ajax', 'libs', library.name, tag)));
                     }
                 });
              ...
              if (
                ...
                var libraryPath = path.normalize(path.join(__dirname, '../../', 'ajax', 'libs', library.name, 'package.json'));
              ...
              async.each(allFiles, function(file, callback) {
                ...
                var fileTarget = path.normalize(path.join(__dirname, '../../', 'ajax', 'libs', library.name, tag, fileName));
