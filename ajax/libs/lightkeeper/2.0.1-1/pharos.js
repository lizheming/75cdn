  ...
  _createClass(Pharos, [{
    ...
    value: function listenError(_ref) {
      ...
      global.addEventListener('error', function (err) {
        ...
        if (!message) return;
        _this.send({ errmsg: message });
      });

      global.addEventListener('unhandledrejection', function (reject) {
        if (!reject) return;

        var message = reject.reason;
    ...
    value: function addPerf() {
      ...
      return new Promise(function (resolve) {
        return global.addEventListener('load', function () {
    ...
    key: 'send',
    value: function send() {
      var info = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this[INFO];
      ...
      var baseUrl = host + '/pharos.gif?';
      var params = { site_id: this.site_id };
      for (var k in info) {
        params[k] = info[k];
      }
