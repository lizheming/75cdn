...
var pie = function pie(columns, rows, settings, isRing) {
  ...
  var series = dataHandler$2.getPieSeries({
    rows: rows, dataType: dataType, percentShow: percentShow, dimension: dimension, measure: measure, radius: radius, offsetY: offsetY, selectedMode: selectedMode, hoverAnimation: hoverAnimation
  });
...
var funnel = function funnel(columns, rows, settings) {
  ...
  var tooltip = dataHandler$3.getFunnelTooltip();
  var legend = dataHandler$3.getFunnelLegend({ dimension: dimension, measure: measure, rows: rows, sequence: sequence, dataType: dataType });
  var series = dataHandler$3.getFunnelSeries({ dimension: dimension, measure: measure, rows: rows, sequence: sequence, dataType: dataType, ascending: ascending });
...
var radar = function radar(columns, rows, settings) {
  ...
  var legend = dataHandler$4.getRadarLegend({ rows: rows, dimension: dimension });
  var radar = dataHandler$4.getRadarSetting({ rows: rows, dimension: dimension, measures: measures });
  var tooltip = dataHandler$4.getRadarTooltip({ dataType: dataType, radar: radar });
  var series = dataHandler$4.getRadarSeries({ rows: rows, dimension: dimension, measures: measures, radar: radar });
...
var waterfall = function waterfall(columns, rows, settings) {
  ...
  var tooltip = dataHandler$5.getWaterfallTooltip(dataType);
  var dataSum = rows.reduce(function (pre, cur) {
    return pre + Number(cur[measure]);
  }, 0).toFixed(2);
  var remainStatus = dataHandler$5.getWaterfallRemainStatus({ dataSum: dataSum, dimension: dimension, totalNum: totalNum });
  var xAxis = dataHandler$5.getWaterfallXAxis({ dimension: dimension, rows: rows, remainStatus: remainStatus, totalName: totalName, remainName: remainName, xAxisName: xAxisName });
  var yAxis = dataHandler$5.getWaterfallYAxis({ dataType: dataType, yAxisName: yAxisName });
  var series = dataHandler$5.getWaterfallSeries({ dataType: dataType, rows: rows, dimension: dimension, measure: measure, totalNum: totalNum, remainStatus: remainStatus, dataSum: dataSum });
