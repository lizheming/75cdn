...
var pie = function pie(columns, rows, settings, extra, isRing) {
  ...
  var tooltip = tooltipVisible && getPieTooltip(dataType, digit);
...
var map = function map(columns, rows, settings, extra) {
  var _settings$position = settings.position,
      ...
      geoItemStyle = settings.geoItemStyle,
      geoLabel = settings.geoLabel;
  ...
  return getMapJSON(position).then(function (json) {
    ...
    return { series: series, tooltip: tooltip, geo: geo, legend: legend };
  });
};
...
var components = [VeBar, VeHistogram, VeLine, VePie, VeRing, VeWaterfall, VeFunnel, VeRadar, VeChart, VeMap];
