...
var waterfall = function waterfall(columns, rows, settings) {
  ...
  var tooltip = dataHandler.getWaterfallTooltip(dataType);
  var dataSum = rows.reduce(function (pre, cur) {
    return pre + Number(cur[measure]);
  }, 0).toFixed(2);
  var remainStatus = dataHandler.getWaterfallRemainStatus({ dataSum: dataSum, dimension: dimension, totalNum: totalNum });
  var xAxis = dataHandler.getWaterfallXAxis({ dimension: dimension, rows: rows, remainStatus: remainStatus, totalName: totalName, remainName: remainName, xAxisName: xAxisName });
  var yAxis = dataHandler.getWaterfallYAxis({ dataType: dataType, yAxisName: yAxisName });
  var series = dataHandler.getWaterfallSeries({ dataType: dataType, rows: rows, dimension: dimension, measure: measure, totalNum: totalNum, remainStatus: remainStatus, dataSum: dataSum });
